...
Single registered node:
    - MeshSplitLTX:  pass-through MODEL node, configures split point +
...
class MeshServerNeedsReconfigure(Exception):
    ...
    button that POSTs /mesh/ltx/reconfigure to apply the change; until
...
# ---------------------------------------------------------------------
# HTTP route: /mesh/ltx/reconfigure
#
...
if _HAS_COMFY_SERVER:
    @_ComfyPromptServer.instance.routes.get("/mesh/ltx/status")
    ...
    @_ComfyPromptServer.instance.routes.post("/mesh/ltx/reconfigure")
...
def _send_node_message(node_id, level: str, text: str) -> None:
    """Send a `mesh-ltx-message` websocket event to ComfyUI's web UI. The
    ...
    try:
        ...
        if inst is None:
            return
        inst.send_sync("mesh-ltx-message", {
            "node_id": str(node_id),
            "level": level,
            "text": text,
        })
...
def _strip_diffusion_back_half(
    ...
    nn.Module in place — see the comment in MeshSplitLTX.configure for
...
def _force_release_block_vram(block: nn.Module) -> int:
    ...
    for name, buf in list(block.named_buffers(recurse=True)):
        try:
            ...
            # Buffers are stored in ._buffers dict on the parent module;
            # the safe in-place replacement is .data = empty just like params.
...
class MeshSplitLTX:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "model": ("MODEL",),
                "n_blocks_remote": ("INT", {"default": 8, "min": 0, "max": 256,
                                            "tooltip": (
                ...
                "codec_mode": (["raw", "Nvenc LTX (5090 optimized)", "nvenc"], {"default": "raw",
                                                  "tooltip": (
                                                      ...
                                                      "PICK BY GPU: on RTX 50-series (Blackwell) on both "
                                                      "ends, use 'Nvenc LTX (5090 optimized)'. On older "
                                                      "cards, try 'raw' or 'nvenc' (whichever you prefer — "
                                                      "raw is the safest baseline; nvenc gives a smaller "
                                                      "wire but on LTX content can sometimes show lower "
                                                      "contrast, so A/B against raw on your workflow before "
                                                      "trusting it). "
                                                      ...
                                                      "on gigabit. Tuned and validated on RTX 5090; "
...
NODE_CLASS_MAPPINGS = {
    "MeshSplitLTX": MeshSplitLTX,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "MeshSplitLTX": "ComfyUI Mesh : Icarus LTX",
}
