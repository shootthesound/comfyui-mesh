...
def build_synthetic_payload(diffusion, device, dtype):
    ...
    Reads shapes from the loaded model rather than hardcoding:
      - n_ada_params from block.scale_shift_table.shape (varies with
        cross_attention_adaln)
      - PE 3-tuples generated by the model's own
        _prepare_positional_embeddings — bulletproof because it's the
        same code path the real forward would use; no risk of
        fabricating cos/sin tensors with the wrong head-dim split.
    """
    ...
    # scale_shift, last row is the gate.
    n_ca_ss, n_ca_gate = 4, 1
    # cross_attention_adaln=True adds prompt_scale_shift_table as a
    # block param (shape (2, v_dim)). Its presence signals we must
    # provide v_/a_prompt_timestep as real tensors (not None).
    ...
    # Plausible token counts. Pixel coords keep things small but the
    # patchifier needs the count to be divisible by the patch size, so
    # we work backwards from a small (frames × H × W) latent.
    B = 1
    # Video: 8 frames × 8x8 spatial patches = 512 tokens. Hardcode here
    # rather than derive from inner_dim — keeps the test fast.
    nF, nH, nW = 8, 8, 8
    T_v = nF * nH * nW
    T_a = 256                          # audio tokens
    C_v, C_a = 256, 64                 # text-context tokens
    ...
    pe_list = diffusion._prepare_positional_embeddings(
        [v_pixel_coords, a_latent_coords], frame_rate, dtype,
    )
    # pe_list is [(v_pe, av_cross_video_freq_cis), (a_pe, av_cross_audio_freq_cis)]
    v_pe_tuple, av_cross_video_freq_cis = pe_list[0]
    a_pe_tuple, av_cross_audio_freq_cis = pe_list[1]
