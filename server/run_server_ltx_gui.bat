...
REM ---- mesh_server_ltx_gui.py reads its mtime in _log_session_header.
echo. > "%~dp0mesh_server_ltx_gui_bat_t0.tmp"

REM ---- Clear stale ready-sentinel so the splash actually waits ----
del /f /q "%~dp0mesh_server_ltx_gui_ready.tmp" >nul 2>nul
...
REM ---- tkinter cold start, and self-closes once mesh_server_ltx_gui.py
REM ---- writes the ready sentinel.
start "ComfyUI Mesh : Daedalus LTX starting" "%~dp0_splash_ltx.cmd"
...
if exist "%VENV_PYW%" (
    ...
    start "" "%VENV_PYW%" "%~dp0mesh_server_ltx_gui.py"
) else (
    "%PY%" "%~dp0mesh_server_ltx_gui.py"
)
