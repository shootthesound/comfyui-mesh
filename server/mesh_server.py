                ...
                if kind == "hello":
                    ...
                    if new_n_blocks == n_total_loaded:
                        print(f"[server] reconfigure no-op: --n-blocks "
                              f"already {n_total_loaded}, staying alive")
                        protocol.send_message(conn, {
...
def _wait_for_vram(device: torch.device,
    ...
    the next load_flux2_klein() crashes mid-load with an OOM stack
    trace, the user manually retries, and the second attempt works
    because by then the kernel has cleaned up. With this probe, the
    server surfaces a clear "waiting..." message and proceeds once
    VRAM is ready (or after max_wait_seconds, in case the user really
    did have something else legitimately holding it).
    """
    if device.type != "cuda":
        return
    try:
